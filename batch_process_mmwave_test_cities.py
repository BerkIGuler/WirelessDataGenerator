"""
Batch Processing Script for mmWave DeepMIMO Scenarios (Test Cities Only)

This script processes downloaded mmWave DeepMIMO scenarios for cities that exist in both:
1. The mmWave download list (city_0_newyork_28, city_10_austin_28, etc.)
2. The processed_channels/test folder (sub6 scenarios)

Processes only the intersection of these two sets to ensure consistency.
...
# =============================================================================
# City Matching Functions
# =============================================================================

def extract_city_name_from_mmwave(scenario_name):
    """
    Extract city name from mmWave scenario name.
    Example: city_0_newyork_28 -> newyork
    """
    match = re.search(r'city_\d+_([^_]+(?:_[^_]+)*)_\d+', scenario_name)
    if match:
        return match.group(1)
    return None

def extract_city_name_from_test_file(filename):
    """
    Extract city name from test file name.
    Example: city_7_sandiego_3p5_bs000.npz -> sandiego
    """
    match = re.search(r'city_\d+_([^_]+(?:_[^_]+)*)_3p5_bs\d+', filename)
    if match:
        return match.group(1)
    return None

def get_matching_cities():
    """
    Find cities that exist in both:
    1. mmWave scenarios (downloaded)
    2. processed_channels/test folder (sub6 scenarios)
    
    Returns:
        list: List of mmWave scenario names to process
    """
    # Get mmWave scenarios
    mmwave_scenarios = []
    if SCENARIOS_DIR.exists():
        for item in SCENARIOS_DIR.iterdir():
            if item.is_dir() and "city" in item.name and item.name.endswith("28"):
                mmwave_scenarios.append(item.name)
    
    # Get test cities
    test_cities = set()
    if TEST_DIR.exists():
        for file in TEST_DIR.glob("city_*_3p5_bs*.npz"):
            city_name = extract_city_name_from_test_file(file.name)
            if city_name:
                test_cities.add(city_name)
    
    # Find matching cities
    matching_scenarios = []
    for mmwave_scenario in mmwave_scenarios:
        mmwave_city = extract_city_name_from_mmwave(mmwave_scenario)
        if mmwave_city and mmwave_city in test_cities:
            matching_scenarios.append(mmwave_scenario)
    
    print(f"Found {len(mmwave_scenarios)} mmWave city scenarios")
    print(f"Found {len(test_cities)} cities in test folder")
    print(f"Found {len(matching_scenarios)} matching cities to process")
    
    # Show breakdown
    if matching_scenarios:
        print(f"\nMatching cities to process:")
        for i, scenario in enumerate(matching_scenarios, 1):
            city_name = extract_city_name_from_mmwave(scenario)
            print(f"  {i:2d}. {scenario} -> {city_name}")
...
def main():
    """Main function to batch process mmWave scenarios for matching test cities."""
    print("DeepMIMO mmWave Scenario Batch Processing (Test Cities Only)")
    print("=" * 70)
    ...
    # Get matching cities
    scenarios = get_matching_cities()
    
    if not scenarios:
        print("No matching cities found. Exiting.")
        return
    ...
    # Process scenarios
    print(f"\nProcessing {len(scenarios)} mmWave scenarios for matching test cities...")
    ...
    for scenario_name in tqdm(scenarios, desc="Processing mmWave scenarios"):
    ...
    # Final summary
    print("\n" + "=" * 80)
    print("MMWAVE BATCH PROCESSING COMPLETE (TEST CITIES ONLY)")
    print("=" * 80)
    
    # Overall statistics
    print(f"Total matching cities found: {len(scenarios)}")
    ...
    if successful_scenarios:
        ...
        for scenario, bs_count in successful_scenarios:
            city_name = extract_city_name_from_mmwave(scenario)
            print(f"  ✓ {scenario} ({city_name}): {bs_count} base stations processed")
    ...
    if len(failed_scenarios) == 0:
        print(f"\n🎉 All mmWave scenarios processed successfully!")
